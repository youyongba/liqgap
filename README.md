...
信号**跟随前端图表当前周期**（`interval` 参数，默认 1h）：K 线、VWAP、ATR、
FVG、CVD 相关性、OI 趋势全部基于所选周期计算。

**路径 ①：FVG 假突破形态（优先级更高 · 仅合约）**

- **做空 `FVG_REJECT_SHORT`**：近 10 根 CVD↑ + 近 6 个样本 OI↑（多头持续堆积）
  \+ 价格上方的**看跌 FVG** 近 6 根内被打进（含瞬间插针）+ 当前又收回其下沿之下
  —— 追多的堆积仓位被闷在上方，转为下跌燃料
- **做多 `FVG_RECLAIM_LONG`**：CVD↓ + OI↑（空头持续堆积）+ 价格下方的
  **看涨 FVG** 被打进 + 当前又收回其上沿之上 —— 下探失败，堆积空头被反杀
- OI 趋势用**币本位持仓数**判断（避免价格涨跌污染名义价值）；命中多个 FVG
  取离当前价最近的；止损锚定被触发的 FVG 远端 ± 当前周期 ATR×倍数
...
- 右侧面板：**📌 关键价位 / Key Levels**（点击任意价格即复制）——
  每个周期 (15m/1h/4h/1d) 的**最近一个已确认**看涨/看跌 FVG 区间
  （确认制：只在已收盘 K 线上检测，排除未收盘 K 线的瞬时缺口；
  主图 FVG 同口径，两边始终同源同值，并附缺口形成时间方便对图）、POC、VWAP，
...
- **📍 关键价位触碰推送**（后台服务 `services/keyLevelsAlert.js`，随 server 启动）：
  ...
  价格**下跌触及看涨 FVG / L↓ 清算主峰 / 买单墙** → 绿色卡片（关注做多）；
  同一轮多个价位合并成一张卡（混合方向为橙色）。防轰炸双保险：触发后
