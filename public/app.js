...
(function () {
  ...
  // 交易信号窗口闸门逻辑已随信号面板移除；三阶窗口闸门（FULL / NOTIFY / 屏蔽）
  // 仍由后端 routes/liqSignal.js + routes/resonanceSignal.js 用 .env 变量控制，
  // 信号轮询由 services/signalPoller.js 在服务端定时执行。
  ...
  // 关键价位 / 清算磁极信号 / 双层共振信号 面板已移除（性能优化）。
  // 信号计算改由后端 services/signalPoller.js 定时轮询，飞书推送 + autoTrade 不受影响。
  ...
  async function poll() {
    ...
    try {
      ...
      const cvdFetch = (market === 'futures' && _cvdAggregate)
        ...
        : Promise.resolve(null);
      // 🧲 清算磁极 / 共振信号已改由后端 signalPoller 定时轮询（飞书 + autoTrade），
      // 前端不再拉取信号接口 → 每轮 poll 少 1-2 个重计算请求。
      const [kData, obData, oiData, cvdData] = await Promise.all([
  ...
  //   curl -X POST http://localhost:3000/api/auto-trade/toggle
  //   curl http://localhost:3000/api/auto-trade/status
