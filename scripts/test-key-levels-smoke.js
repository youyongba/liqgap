...
// ---------------------------------------------------------------------------
// 场景 1b：FVG 确认制 —— 未收盘 K 线的瞬时缺口不算，取最近已确认缺口
// ---------------------------------------------------------------------------
console.log('\n[1b] _computeIntervalLevels · 确认制（排除未收盘 K 线）');
{
  const t0 = 1700000000000;
  // 结构：bars 0-2 形成已确认的 bearish FVG [97,99]；
  // 最后一根（未收盘）bar5 与 bar3 之间出现"瞬时"bearish 缺口
  // [95.5 ~ 95.8]（bar3.low=95.8 > bar5.high=95.5）→ 必须被排除。
  const raw = mkRaw([
    [100, 101, 99, 100, 50],      // bar0: low=99
    [98, 98.5, 96.5, 97, 80],     // bar1
    [96.8, 97, 95.5, 96, 120],    // bar2: high=97 → 确认 bearish FVG [97,99]
    [96, 97.5, 95.8, 97.2, 60],   // bar3: low=95.8
    [97.2, 100.5, 97, 100, 70],   // bar4（涨破 99，但"最近一个"口径仍返回它）
    [95, 95.5, 94, 94.5, 90]      // bar5（未收盘）: high=95.5 → 瞬时缺口
  ], 3600_000, t0);
  const r = kl._computeIntervalLevels('1h', raw);
  check('未收盘 K 线的瞬时缺口被排除', !(r.bearFvg && r.bearFvg.upper < 96));
  check('返回最近已确认的看跌 FVG [97,99]',
    r.bearFvg && r.bearFvg.lower === 97 && r.bearFvg.upper === 99);
}
